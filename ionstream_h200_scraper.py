...
class IonStreamH200Scraper:
    def __init__(self):
        self.name, self.url = "IonStream", "https://ionstream.ai/"
        ...
        if r.status_code == 200:
            ...
            if 'H200' in text:
                for p in re.findall(r'H200[^\$]*\$([0-9.]+)', text, re.I):
                    if 0.5 < float(p) < 20: return {"H200 (IonStream)": f"${float(p):.2f}/hr"}
        return {}
    ...
    def save_to_json(self, prices: Dict[str, str], filename="ionstream_h200_prices.json"):
        ...
        with open(filename, 'w') as f:
            json.dump({"timestamp": time.strftime("%Y-%m-%d %H:%M:%S"), "provider": self.name,
                      "providers": {"IonStream": {"name": "IonStream", "url": self.url,
                      "variants": {"H200 (IonStream)": {"gpu_model": "H200", "gpu_memory": "141GB",
                      "price_per_hour": pv, "currency": "USD", "availability": "on-demand"}}}}}, f, indent=2)
        print(f"💾 Saved: {filename}")

if __name__ == "__main__":
    IonStreamH200Scraper().get_h200_prices()
