#!/usr/bin/env python3
"""
Fal.ai H200 GPU Price Scraper
Extracts H200 pricing from fal.ai pricing page

Fal.ai offers H200 GPUs (141GB) at serverless pricing.
...
class FalaiH200Scraper:
    """Scraper for Fal.ai H200 GPU pricing"""
    
    def __init__(self):
        self.name = "Fal.ai"
        self.base_url = "https://fal.ai/pricing"
        self.headers = {
    ...
    def get_h200_prices(self) -> Dict[str, str]:
        """Main method to extract H200 prices from Fal.ai"""
        ...
        methods = [
            ("Fal.ai Pricing Page Scraping", self._try_pricing_page),
        ...
        if not h200_prices:
            print("\n❌ Failed to extract H200 pricing from Fal.ai")
            return {}
        ...
        for variant, price_str in prices.items():
            ...
            try:
                ...
                if price_match:
                    ...
                    # Fal.ai H200 pricing is around $2.10/hr
                    if 1.0 < price < 5.0:
                        return True
            except:
                continue
    ...
    def _try_pricing_page(self) -> Dict[str, str]:
        """Scrape the Fal.ai pricing page for H200 prices"""
        h200_prices = {}
    ...
    def _extract_from_tables(self, soup: BeautifulSoup) -> Dict[str, str]:
        ...
        for table in tables:
            ...
            # Only process tables with H200 mentions
            ...
            for row in rows:
                ...
                if 'H200' in row_text:
                    ...
                    # Table structure: GPU | VRAM | Price/h | Price/s
                    # The price per hour is in the 3rd cell
                    if len(cells) >= 3:
                        # Find the cell with /h pricing
                        for cell in cells:
                            cell_text = cell.get_text().strip()
                            if '/h' in cell_text and '$' in cell_text:
                                print(f"         Price cell: {cell_text}")
                                price_match = re.search(r'\$([0-9.]+)/h', cell_text)
                                if price_match:
                                    try:
                                        price = float(price_match.group(1))
                                        if 1.0 < price < 5.0:
                                            variant_name = "H200 141GB (Fal.ai)"
    ...
    def _extract_from_text(self, text_content: str) -> Dict[str, str]:
        ...
        # Format: "H200 141GB $2.10/h"
        patterns = [
            r'H200[^\$]*\$([0-9.]+)/h',
            r'H200[^\$]*141GB[^\$]*\$([0-9.]+)',
            r'H200[^\$]*\$([0-9.]+)\s*(?:per\s+hour|/hr|/hour)',
        ]
    ...
    def _try_selenium_scraper(self) -> Dict[str, str]:
        """Use Selenium to scrape JavaScript-loaded pricing from Fal.ai"""
        h200_prices = {}
        ...
        try:
            ...
            try:
                print(f"    Loading Fal.ai pricing page...")
                ...
                script = """
                    ...
                    const gpuTable = tables.find(t => t.innerText.includes('H200'));
                    if (!gpuTable) return null;
                    
                    const h200Row = Array.from(gpuTable.querySelectorAll('tr')).find(tr => tr.innerText.includes('H200'));
                    if (!h200Row) return null;
                    
                    const cells = h200Row.querySelectorAll('td');
                ...
                if result:
                    print(f"    ✓ Found price cell: {result}")
                    price_match = re.search(r'\$([0-9.]+)/h', result)
                    if price_match:
                        price = float(price_match.group(1))
                        if 1.0 < price < 5.0:
                            h200_prices["H200 141GB (Fal.ai)"] = f"${price:.2f}/hr"
                            print(f"    ✓ Extracted: ${price:.2f}/hr")
                else:
                    ...
                    if found_prices:
                        ...
                        found_prices = self._extract_from_text(soup.get_text())
                        if found_prices:
                            h200_prices.update(found_prices)
    ...
    def _normalize_prices(self, prices: Dict[str, str]) -> Dict[str, str]:
        ...
        print("\n   📊 Normalizing Fal.ai H200 pricing...")
    ...
    def save_to_json(self, prices: Dict[str, str], filename: str = "falai_h200_prices.json") -> bool:
        ...
        try:
            ...
            output_data = {
                ...
                "notes": {
                    "instance_type": "Serverless GPU",
                    "gpu_model": "NVIDIA H200",
                    "gpu_memory": "141GB HBM3e",
                    "gpu_count_per_instance": 1,
                    "pricing_type": "Serverless (Pay-per-use)",
...
def main():
    """Main function to run the Fal.ai H200 scraper"""
    print("🚀 Fal.ai H200 GPU Pricing Scraper")
    print("=" * 80)
    print("Note: Fal.ai offers H200 GPUs at serverless pricing")
    print("=" * 80)
    
    scraper = FalaiH200Scraper()
